...
describe('RestClient Enhanced Features', () => {
  ...
  describe('HTTP Methods', () => {
    it('should make GET requests', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('https://discord.com/api/v10/guilds/123', {
        method: 'GET',
        headers: {
    ...
    it('should make POST requests with JSON body', async () => {
      ...
      expect(result).toEqual(mockResponse);
      expect(mockFetch).toHaveBeenCalledWith('https://discord.com/api/v10/channels/123/messages', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bot test-bot-token'
  ...
  describe('Form Data Upload', () => {
    it('should upload form data without Content-Type header', async () => {
      ...
      mockFormData.append('payload_json', JSON.stringify({ content: 'File upload' }));
      mockFormData.append('files[0]', new Blob(['test content'], { type: 'text/plain' }), 'test.txt');

      mockFetch.mockResolvedValue({
        ok: true,
        status: 200,
        json: () => Promise.resolve({ id: '789', attachments: [{ filename: 'test.txt' }] }),
        headers: new Headers()
      });

      const result = await client.postFormData('/channels/123/messages', mockFormData);
